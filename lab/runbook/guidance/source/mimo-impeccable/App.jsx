...
export default function App() {
  ...
  /* The runs that still need a human: status is not Passed and the result is
     not already open in the selected-run panel. */
  const pending = filtered.filter((run) => run.status !== 'Passed' && run.id !== selectedId);
  ...
  const listVisible =
    listState !== 'loading' && listState !== 'error' && listState !== 'empty';
    ...
    <main className="shell">
      ...
      <header className="page-head">
        ...
        <p className="page-note mono">
          {RUNS.length} runs · {CHECKS_PASSED} of {CHECKS_TOTAL} checks · {CLOCK} · {SPEND}
        </p>
      </header>

      {listVisible && pending.length > 0 && (
        <section className="attention" aria-label="Needs attention">
          <p className="eyebrow attention-label">Needs attention</p>
          <ul className="attention-list">
            {pending.map((run) => (
              <li key={run.id}>
                <button
                  type="button"
                  className={`attention-card attention-card--${TONE[run.status]}`}
                ...
                >
                  <span className="attention-copy">
                    ...
                    <span className="attention-text">
                      ...
                      <span className="attention-facts">
                        <span>{run.ref}</span>
                        <span>{run.checks} of {run.total} checks</span>
                        <span>{run.started}</span>
                      </span>
                    </span>
                  </span>
                  <span className="attention-cta">
                    <span className="mono">Inspect run</span>
