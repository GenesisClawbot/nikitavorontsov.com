...
function dd(e) {
	let t = e.querySelector(".portrait-rig"), n = e.querySelector(".portrait-window img"), r = {
		damping: 28,
		stiffness: 150,
		mass: 1
	}, i = cd(0, r), a = cd(0, r), o = cd(0, r), s = cd(0, r), c = ed(t, {
		rotateX: i,
		rotateY: a
	}), l = ed(n, {
		x: o,
		...
		if (t.pointerType === "touch") return;
		let n = e.getBoundingClientRect(), r = Math.max(-1, Math.min(1, (t.clientX - n.left) / n.width * 2 - 1)), c = Math.max(-1, Math.min(1, (t.clientY - n.top) / n.height * 2 - 1));
		i.set(-c * 7), a.set(r * 8), o.set(r * 5), s.set(c * 3);
	};
	return e.addEventListener("pointermove", d, { passive: !0 }), e.addEventListener("pointerleave", u), e.addEventListener("pointercancel", u), () => {
		e.removeEventListener("pointermove", d), e.removeEventListener("pointerleave", u), e.removeEventListener("pointercancel", u), c(), l(), [
			i,
			a,
			o,
			s
		].forEach((e) => e.destroy()), t.style.removeProperty("transform"), n.style.removeProperty("transform");
	};
}
function fd() {
	let e = matchMedia("(hover: hover) and (pointer: fine)"), t = !1, n = [], r = () => {
		...
		let r = document.querySelector(".hero-portrait");
		t && e.matches && r && n.push(dd(r));
	};
...
//#region src/home.mjs
hi.registerPlugin(Q);
var pd = document.querySelector(".journey"), md = [...document.querySelectorAll(".scene")], hd = [...document.querySelectorAll("[data-chapter]")], gd = [...document.querySelectorAll(".motion-toggle")], _d = document.querySelector("#art-reveal"), vd = matchMedia("(prefers-reduced-motion: reduce)"), yd = matchMedia("(min-width: 800px) and (min-height: 640px)"), bd = (e, t = 0, n = 1) => Math.min(n, Math.max(t, e)), xd = (e, t, n) => {
	let r = bd((n - e) / (t - e));
	return r * r * (3 - 2 * r);
}, Sd;
try {
	Sd = localStorage.getItem("nv-motion");
} catch {}
var Cd = !vd.matches && Sd !== "off", wd, Td, Ed = !1, Dd = 0, Od = -1, kd = fd(), Ad = null, jd, Md = {
	from: 0,
	to: 0,
	mix: 0,
	paint: 1,
...
async function Id() {
	...
	try {
		...
		Td = n, Td.setState(Md), Td.setActive(wd.isActive), pd.classList.add("webgl-ready"), pd.dataset.renderer = "webgl";
	} catch (e) {
		pd.dataset.renderer = "static", console.info("Portfolio: using the static artwork sequence.", e.message);
	} finally {
		e === Dd && (Ed = !1);
	}
}
...
})), gd.forEach((e) => e.addEventListener("click", () => {
	Cd = !Cd;
	try {
